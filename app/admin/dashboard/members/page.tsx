...
export default function Members() {
  ...
  if (loading) return <div className="p-4 flex items-center justify-center h-screen"><p className='text-content-day text-xl'>Loading</p>
   <Loader2 className='animate-spin size-8 text-primary-active' /></div>
  ...
  return (
    <div className="flex flex-col gap-2">
       <div className="border-b-[1px] bg-[#f7f8f8] border-b-outline-day h-16 w-full flex items-center justify-between px-7">
         ...
         <button onClick={() => setSelectedFilter('active')} className={getButtonClass('active')}>Active ({filterCounts.active})
         </button>
         <button onClick={() => setSelectedFilter('inactive')} className={getButtonClass('inactive')}>
          Inactive ({filterCounts.inactive})
         </button>
         <button onClick={() => setSelectedFilter('verified')} className={getButtonClass('verified')}>Verified ({filterCounts.verified})</button>
         <button onClick={() => setSelectedFilter('unverified')} className={getButtonClass('unverified')}> Unverified ({filterCounts.unverified})</button>
         <button onClick={() => setSelectedFilter('new')} className={getButtonClass('new')}>
         New Members ({filterCounts.new})</button>
         </div>
